...
from DenRoze3_models import Item, BillItem, Order, Bill, User
import sqlite3
...
class Sqlite_db:
    ...
    def create_database(self):
        ...
        self.create_table(Stock_script)
        ...
        self.create_table(Order_script)
        ...
        self.create_table(BillItem_script)
        ...
        self.create_table(OrderItem_script)
...
class Local_db:
    ...
    def write_changes(self, stock, bills, orders):
        with open(self.path_to_stock_history, "w+") as file:
            items = []
            for item in stock:
                items.append(item.__dict__)
            json.dump(items, file, indent=2)
        with open(self.path_to_bills_history, "w+") as file:
            bill_list = []
            for bill in bills:
                bill_list.append(bill.transform())
            json.dump(bill_list,file, indent=2, default=str)
        with open(self.path_to_orders_history, "w+") as file:
            order_list = []
            for order in orders:
